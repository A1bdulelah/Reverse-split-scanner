...
st.caption("بيانات تحليلية فقط — ليست توصية شراء أو بيع.")
...
def main():
    ...
    with st.spinner("جاري تحميل البيانات..."):
        try:
            ...
            st.error(f"تعذر تحميل بيانات السعر: {e}")
            return

        split = latest_reverse_split(splits)
        if split is None:
            st.warning("لم يتم العثور على Reverse Split ضمن البيانات المتاحة.")
            return
    ...
    if ibd.empty:
        st.info("لا توجد بيانات IBorrowDesk متاحة لهذا السهم.")
    else:
