...
def get_iborrowdesk(ticker):
    ...
    # The public report page exposes a Recent readings table.
    tables = []
    try:
    ...
    for table in tables:
        ...
        if "reported" in cols and ("fee" in cols or "borrow fee" in cols) and any("available" in c for c in cols):
            ...
            if not t.empty:
                cutoff = pd.Timestamp.now(tz=None) - pd.Timedelta(days=31)
                t["reported"] = t["reported"].dt.tz_localize(None)
                t = t[t["reported"] >= cutoff].reset_index(drop=True)
                return t

    # Fallback: extract visible "Recent readings" rows from page text.
    text = re.sub(r"<[^>]+>", " ", html)
...
def main():
    ...
    st.caption("IBorrowDesk data is sourced from its public report page. Availability can vary by ticker and by the site's current data window.")
