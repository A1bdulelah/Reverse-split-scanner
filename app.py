...
HEADERS = {
    "User-Agent": "ReverseSplitScanner/1.2",
...
def get_iborrowdesk(ticker):
    ...
    # IBorrowDesk has used different top-level keys over time.
    if isinstance(payload, dict):
        records = payload.get("daily") or payload.get("real_time") or payload.get("data") or []
    ...
    for item in records:
        if not isinstance(item, dict):
            continue
        date = pd.to_datetime(item.get("date") or item.get("reported"), errors="coerce")
        if pd.isna(date):
            continue
        fee = item.get("fee")
        ...
        rows.append({
            "date": date,
            "fee": pd.to_numeric(fee, errors="coerce"),
            "available": pd.to_numeric(available, errors="coerce"),
        })
...
def main():
    ...
    if sec.get("error") and (sec["shares"] is None or not sec["country"]):
        st.caption("SEC/Yahoo company-profile data was unavailable for this ticker at the time of the request.")

    st.divider()
    st.caption("Price/split/RSI: Yahoo Finance • Company profile/shares: SEC EDGAR + Yahoo Finance")
    st.subheader("IBorrowDesk — 1M")

    if ibd.empty:
        st.info("No IBorrowDesk data is available for this ticker.")
        st.caption(f"IBorrowDesk source: https://www.iborrowdesk.com/report/{ticker}")
    else:
